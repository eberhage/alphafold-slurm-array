...
if [[ -n "${INFERENCE_STATISTICS_FILE:-}" ]]; then
    ...
    echo "gpu_profile,inference_id,inference_name,job_id,task_id,node,tokens,bucket_size,iptm,ptm,ranking_score,start_time,end_time" > "$INFERENCE_STATISTICS_FILE"
fi
